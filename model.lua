...
local function build_lookup_bow(context, input, hop)
   ...
   --context (#batch, hidsz)
    ...
    local Aout2dim = nn.View(g_opts.batch_size, math.max(1,g_opts.memsize))(Aout) --(#batch*1, memsize)
    local P2dim = nn.SoftMax()(Aout2dim) --(#batch*1, memsize)
    local P = nn.View(g_opts.batch_size, 1, math.max(1,g_opts.memsize))(P2dim) --(#batch, 1, memsize)
...
function g_build_model()
    ...
    local model = nn.gModule(input_table, {action_logprob, baseline, P})
